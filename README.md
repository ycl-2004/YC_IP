...
无论用户怎么措辞——哪怕明确说"**结构图 / 架构图 / 解释图 / 流程图 / diagram**"——本 skill 永远**不画方框 + 箭头的论文式结构图**。必须把抽象结构**重新翻译成一个 YC 正在亲手操作的手绘物理隐喻场景**：可以是机器/织机，也可以是桥、路径、分拣台、反馈循环、接力校准、桌面专注、保护伞、倾听笔记、情绪转译或小胜利场景。先选场景原型，再让信息走完"具体起点 → YC 操作 → 可见变化/结构 → 验证/回应/结果"。

- 看到"结构图/架构图"= 触发**改写**,不是触发画框图。先在脑内完成 [`yc-ip/references/concept-director.md`](yc-ip/references/concept-director.md) 的"不是 A,而是 B"机制判断 + 物理动词，再读 [`yc-ip/references/explanation-variation.md`](yc-ip/references/explanation-variation.md) 选择场景原型。**抽象/知识主题强制先读 concept-director 和 explanation-variation。**
...
- 每次先扫描 `yc-ip/assets/examples/` 和 `examples/images/`，按 topic、比例、情绪、信息密度和场景原型选择真实 primary reference；它只锁定画面语言，不复制构图。`07-noise-filter.png` 是单角色机制图 fallback，不是固定默认。正式产物必须把选中的 asset 作为真实 image input。

> 一句话:用户要的"Transformer 结构图"= YC 正在操作的"注意力织机/装配机"；用户要的"信任怎么建立"= YC 铺证据桥；用户要的"如何保护专注"= YC 撑起透明伞挡住干扰。都不是论文里的方框图。
...
- 先从 `explanation-variation.md` 选一个场景原型，不要自动套机器。
- 只画一个连续场景：具体起点 → 一个 YC 操作 → 可见变化/结构 → 验证/回应/结果。
...
例：解释 Machine Learning 这类内部预测机制时，可以画 YC 把分好类的水果例子倒进手摇学习机，机器把特征织成规则网，再用一个新水果测试并输出“苹果？”预测。解释信任、反馈、专注、情绪、用户需求时，优先使用桥、循环、桌面/保护、音乐转译、倾听笔记等不同场景。
...
- Sample / Standard → 先按 `yc-ip/references/reference-routing.md` 和 `yc-ip/references/explanation-variation.md` 扫描并选择 topic-matched primary reference；`07-noise-filter.png` 只在没有更贴切主题图时作为单角色机制图 fallback。
...
Broad Concept 不要固定使用 `05-concept-explainer.png`、`07-noise-filter.png`、多角色图、Rich 图或九宫格作为默认参考。先选场景原型，再选对应 example。第一次仍可能偏；如果出现深色背景、霓虹、多 YC、标题段、面板、UI、底部总结条，或主题不适合机器却仍画成机器，直接判失败并用更贴切的 canonical 参考重生成，最多自动重试两次。
...
这些图片是成品展示样例。使用时优先学习它们的场景语言：YC 参与核心动作，方法可以变成桥、分拣装置、输出装置、循环、接力、桌面、保护、倾听、情绪转译或小胜利；不要做成 PPT 流程图，也不要把角色贴在白色小方块里。
...
4. 读取 `yc-ip/references/reference-routing.md`；解释类再读 `yc-ip/references/explanation-variation.md`，扫描 examples 后取 topic-matched primary reference；`07-noise-filter.png` 仅作 fallback
...
- **每次生图都带参考图**，这是稳定性的关键
- 每张图都先选 topic-matched primary reference；解释类图片先选场景原型。`07-noise-filter.png` 仅在没有更贴切主题图时作为机制图 fallback。九宫格只在角色连续两次漂移时作为第二张修复参考
...
本仓库为 YC 的个人作品，**版权归 YC 所有**。源码与素材公开仅供浏览与参考，**未经书面许可，不得复制、修改、再分发或用于商业用途**。「YC」角色形象、画风、配色与品牌资产为 YC 专有，**不在任何开源 / 共享授权范围内**。

This is YC's personal work, public for viewing only — no copying, modifying,
redistribution, or commercial use without written permission. The YC character,
art style, and brand assets are proprietary and are **not** open-source or
shared. See [LICENSE](LICENSE) · Contact: yichen.lin.2004@gmail.com
