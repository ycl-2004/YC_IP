# YC Personal IP Illustrations

> 用 YC 的 IP 角色为任何内容生成配图。默认是干净的水彩铅笔小贴纸，需要时也能做多步故事流或满版 editorial。
>
> 三种模式（Sticker / Simple / Rich）| 多比例支持 | YC chibi 角色（红发透明眼镜）| 必带参考图工作流 | Claude Code Skill

---

## 这个仓库是什么

YC Personal IP Illustrations 是一个 Claude Code Skill，用来指导 AI 用 YC 的固定 IP 角色生成个人品牌配图。

它不是通用插画 prompt，也不是 PPT 模板。核心目标是：把任何内容（YC 的5大主题方向，或任意知识主题）用 YC 的视觉风格和角色呈现出来，生成一张有 YC 品牌识别度的图。

默认视觉 IP 是「YC」：一个红发蓬松、戴透明圆框眼镜、chibi 比例的年轻人角色。YC 不是吉祥物，不是装饰，而是正在认真探索世界、build 东西、创作音乐、持续学习的真实年轻人缩影。

一句话：**让每一张图都有 YC 的识别度，让内容和角色浑然一体。**

---

## 三种输出模式（核心）

这个 skill 不是单一风格，而是一个**从轻到重的密度光谱**。默认永远偏轻，避免把图做成满版海报。

| | Sticker 贴图（默认） | Simple 普通 | Rich 丰富 |
|---|---|---|---|
| **样子** | 白底水彩铅笔小贴纸，单角色 | 多步场景故事流 | 满版 editorial，多面板 |
| **画风** | 水彩铅笔轻涂松手绘 | 温暖 chibi 插画 | 精致 editorial 插画 |
| **背景** | 纯白 | 奶油白 | 奶油白 |
| **文字** | 0-3 个中文短词 | 每步 1-2 个短标注 | 中英混排，多标注 |
| **何时用** | 文章正文配图、社媒单图、一个概念/情绪、封面/标题 | 流程、步骤、使用方法、before-after | IP 自我介绍、品牌全景、一张讲完整个体系 |

**自动判断口诀**（用户没指定时）：

- **一件事** → Sticker（默认）
- **多步过程 / 对比** → Simple
- **整页就是成品**（IP 介绍 / 品牌全景）→ Rich

> **Rich 是例外，不是常态。** 日常文章和社媒配图几乎都在 Sticker / Simple 之间。封面/标题也走 Sticker（封面版：一个 hero + 一行标题 + 白底），不是 Rich。怀疑时永远选更轻的那个。

想手动指定时直接说模式词即可：「用 sticker 画…」「这个 simple 分步…」「来个 rich 的 IP 介绍」。

---

## 生图必须带参考图（最重要的稳定性技巧）

纯文字 prompt 不稳，模型会回退到"精致全彩 anime + 满版拼贴"的海报先验。**每次生图都附一张参考图。**

参考图同时锁两件事：**①YC 长相 ②画风渲染**。

- Sticker 模式 → 附 `yc-personal-ip/assets/examples/sticker/` 里的水彩贴纸（如 `03-sitting-with-laptop.png`、`05-holding-smiley-mug.png`），两件一起锁。
- Simple / Rich 模式 → 附 `simple/` 或 `rich/` 里对应模式的图。
- 只想锁角色长相 → 附 `character-reference/` 里的九宫格（注意：九宫格是全彩，只能锁长相、锁不了水彩画风）。

第一次仍可能偏 → 正常，带参考图重生成第二次就对。换 prompt 里的「动作/场景」那一行，就是同一个 YC 干不同的事。

---

## 适合什么场景

**最适合：**

- 为 YC 的原创文章、知识帖、小红书/B站内容生成干净的正文配图与封面
- 想要统一视觉风格、固定 IP 角色的个人品牌内容创作者
- 需要多比例配图（16:9 文章 / 1:1 社媒 / 9:16 竖版 Story 等）
- 想要轻量、留白干净、像手账水彩小贴纸的配图（而不是信息塞满的海报）

**不适合：**

- 想要商业品牌 KV 或精修商业插画的人
- 想要严格可编辑矢量源文件的人
- 想要大量文字塞满画面的信息图的人

---

## 它会产出什么

- 指定比例的 YC IP 配图（支持 8 种比例）
- 文章/内容的 shot list（每张图的主题、模式、YC 动作、标注建议）
- 最终 PNG，保存到 `assets/<content-slug>/`

**支持的图片比例：**

| 比例 | 用途 |
|------|------|
| `16:9` | 文章配图、博客 banner、B站封面 |
| `1:1` | Instagram、小红书方图、通用社媒 |
| `9:16` | Story、抖音竖版、朋友圈竖图 |
| `4:3` | 传统横版、部分平台帖子 |
| `3:2` | 横版照片风格、博客图 |
| `2:3` | Pinterest、竖版海报 |
| `5:4` | 微博/部分平台方形偏横 |
| `21:9` | 超宽 banner、网站 header |

**不输出：** PPTX / PDF / 可编辑源文件、商业插画 KV、无 YC 角色的通用插画。

---

## 示例效果

### Sticker 贴图（默认）— 白底水彩铅笔单角色
![YC Sticker](yc-personal-ip/assets/examples/sticker/03-sitting-with-laptop.png)

### Simple 普通 — 干净的轻场景 / 标题图
![YC Simple](yc-personal-ip/assets/examples/simple/04-titled-scene-with-notes.png)

### Rich 丰富 — IP 自我介绍（满版仅此类用）
![YC Rich](yc-personal-ip/assets/examples/rich/01-ip-character-sheet.png)

> 这些图片是风格校准 + 参考图。使用时从当前内容重新设计场景，不要直接复刻已有构图。

---

## 安装

```bash
git clone https://github.com/<your-github>/yc-personal-ip.git
cd yc-personal-ip
cp -R ./yc-personal-ip ~/.claude/skills/yc-personal-ip
```

安装后，在 Claude Code 或 claude.ai 里使用 `/yc-personal-ip`。

---

## 怎么用

### 配图策略（先不生图）

```text
/yc-personal-ip 先不要生图。
分析下面这篇文章哪里值得配图，输出 shot list。
每张图写清楚：段落位置、主题、核心意思、用哪个模式、YC 在做什么、建议比例、建议标注词。

<粘贴文章>
```

### 文章正文配图（默认 Sticker）

```text
/yc-personal-ip 为下面这篇文章配几张图（默认贴纸风格，白底干净）。

<粘贴文章>
```

### 单个概念 / 情绪（Sticker）

```text
/yc-personal-ip 画一张「专注写作」的贴纸图，1:1。
```

### 文章封面 / 标题图（Sticker 封面版）

```text
/yc-personal-ip 给这篇文章做个 16:9 封面，标题「我是怎么搭出个人 IP 的」。
一个 YC hero + 一行标题 + 白底，干净不堆元素。
```

### 流程 / 步骤（Simple）

```text
/yc-personal-ip 用 simple 模式把这个学习方法画成 3-5 步的流程图，16:9。
```

### IP 自我介绍（Rich）

```text
/yc-personal-ip 生成一张 16:9 的 YC IP 介绍图，展示 YC 是谁、关注什么、在做什么。用 rich 模式。
```

更多示例见 [examples/prompts.md](examples/prompts.md)。

---

## 工作流程

1. 读取用户给的内容（文章/帖子/想法/知识主题）
2. 先提"认知锚点"——只给核心判断/转折/对比/闭环配图，够用就好，不做画册
3. 按"一件事 / 多步过程 / 整页成品"判断用哪个模式（默认 Sticker）
4. 取对应模式的参考图
5. 每张图单独调用图像模型生成（带参考图）
6. QA 检查：红发 + 透明眼镜（所有模式必须）；按模式查背景与密度（详见 `references/qa-checklist.md`）
7. 保存 PNG，报告用途、模式和路径

---

## 目录结构

```text
.
├── README.md
├── LICENSE
├── examples/
│   └── prompts.md               # 更多使用示例
└── yc-personal-ip/              # Claude Code Skill 主体（需要安装的就是这个）
    ├── SKILL.md
    ├── agents/
    │   └── openai.yaml
    ├── assets/
    │   └── examples/            # 风格校准 + 参考图，按模式分文件夹
    │       ├── sticker/         # 默认：白底水彩贴纸（含动作 + 穿搭变体）
    │       ├── simple/          # 多步场景流 / 干净标题图
    │       ├── rich/            # 满版 editorial（IP/品牌）
    │       └── character-reference/  # 九宫格：锁角色长相 + 姿势库（浏览挑姿势）
    └── references/
        ├── style-dna.md         # 三模式风格 DNA、防海报护栏
        ├── yc-character.md      # YC 角色外形、服装、生图关键词（含贴图轻渲染版）
        ├── yc-brand-identity.md # 5大内容支柱、人设、signature quotes
        ├── composition-patterns.md
        ├── prompt-template.md   # 三模式 prompt + 封面版 + 参考图工作流
        └── qa-checklist.md      # 按模式的生成后检查
```

---

## 注意事项

- YC 必须有**红发**和**透明圆框眼镜**，缺一不可，否则角色识别失败需重生成
- **每次生图都带参考图**，这是稳定性的关键
- 一张图只讲一件事，宁可多出几张轻图，也不要一张塞满
- 背景按模式：**Sticker 纯白**；Simple / Rich 奶油白。别套错（Sticker 套奶油白 + 道具就会变海报）
- `assets/examples/` 只用来锁画风和角色，不要照抄具体构图
- AI 图像模型可能出现角色漂移（发色不对/眼镜丢失），生成后需检查
- 中文标注越短越稳定

---

## 关于 YC

**YC (Yichen)** — AI Builder / Music Creator / Always Learning

用创意和技术 build 有意思的东西。

- 内容方向：AI 工具 × 个人成长 × 音乐创作 × 设计创意 × 留学生活
- 平台：小红书 / B站 / YouTube / Instagram
- Motto：认真生活，浪漫创作。♥

---

## License

MIT License. See [LICENSE](LICENSE).
