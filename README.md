...
- 参考这张正确范例的"语言而非构图"(白底、手绘、YC 操作装置、少量批注):`yc-ip/assets/examples/simple/current-showcase/07-noise-filter.png`。**即使你的环境无法把这张图作为附件喂给生图工具(如 web 浏览场景),也要靠下方 prompt-template 里完整的文字 Visual DNA 独立成图——那段 DNA 是按"无参考图也成立"写的。**
...
- Sample / Standard Broad Concept → 固定附 `yc-ip/assets/examples/simple/current-showcase/07-noise-filter.png`；它是单角色、白底、单场景 canonical style lock。
