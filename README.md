...
- 看到"结构图/架构图"= 触发**改写**,不是触发画框图。先在脑内完成 [`YC_IP/references/concept-director.md`](YC_IP/references/concept-director.md) 的"不是 A,而是 B"机制判断 + 物理动词 + 低科技装置,再生图。**抽象/知识主题强制先读 concept-director。**
...
- 参考这张正确范例的"语言而非构图"(白底、手绘、YC 操作装置、少量批注):`YC_IP/assets/examples/simple/current-showcase/07-noise-filter.png`。**即使你的环境无法把这张图作为附件喂给生图工具(如 web 浏览场景),也要靠下方 prompt-template 里完整的文字 Visual DNA 独立成图——那段 DNA 是按"无参考图也成立"写的。**
...
- Sample / Standard Broad Concept → 固定附 `YC_IP/assets/examples/simple/current-showcase/07-noise-filter.png`；它是单角色、白底、单场景 canonical style lock。
...
4. 取对应模式的参考图；Broad Concept 固定用 `07-noise-filter.png`
...
- **每次生图都带参考图**，这是稳定性的关键
- Broad Concept 默认只带 `07-noise-filter.png`；多角色、多场景、九宫格和 Rich 图不能作为默认风格锁
