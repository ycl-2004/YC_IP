---
name: yc-ip
description: 生成 YC 风格的中文个人 IP 配图。调用本 skill、要求“用 YC 解释 X”或“做一张 X 的图”时，在 Codex 里默认使用 Codex native image_gen 生图能力生成并交付图片，不要求 OPENAI_API_KEY；本地 imagegen CLI/API 只作为用户明确要求或 native route 不可用时的 fallback。不能只输出分析、画面方案或生图 prompt；只有用户明确说只分析、只要 prompt、shot list 或不要生成时才停在文字。默认使用 Sample/Standard 成品风格：纯白底、YC 红发透明眼镜 chibi、手绘线条、中文短标注、中等信息密度。只有用户明确点名 Minimal/Sticker/贴图纸/贴纸/极简/最小贴纸时才启用 Minimal；只有明确点名 Rich、完整海报、IP 设定页或品牌全景时才启用 Rich。默认比例：文章 16:9 / 社媒 1:1。
---
...
**正式产物默认走 Codex native image route**：在 Codex 环境里，正式 YC 图片默认使用系统 `imagegen` skill 的 built-in `image_gen` 工具，不要求 `OPENAI_API_KEY`。本地 examples PNG 仍必须作为真实参考进入上下文：先用 `view_image` 打开 selected primary reference（以及必要的 continuity anchor），再调用 built-in `image_gen`，生成后从 `$CODEX_HOME/generated_images/...` 复制最终 PNG 到 `YC_Img_Out/`。`image_gen.py` CLI/API 只作为用户明确要求 API/CLI 路线、或 built-in route 不可用时的 fallback；fallback 才需要 `OPENAI_API_KEY`。

## 交付物闸门（最高优先级）

本 skill 的默认交付物是**生成完成的图片**，不是文字解释、构图方案或最终 prompt。

- 用户调用本 skill，或说“用 YC 解释 X / 什么是 X / 做一张 X 的配图 / 跟这个风格一样”时：完成内部构思后，必须在同一轮使用 Codex native `image_gen` 生成图片。
- 正式产物不得因为缺少 `OPENAI_API_KEY` 停止；API key 只属于 CLI/API fallback。
- “概念解释、画面隐喻、短标注、最终 illustration prompt”都只是内部中间产物；不要把它们当作任务完成状态。
- 只有用户明确说“只分析 / 先分析 / 只要方案 / 只要 prompt / shot list / 不要生成图片”时，才输出文字并停止。
- 如果 Codex native `image_gen` 不可用，才说明 fallback 选项；用户明确选择 CLI/API fallback 后再使用 `image_gen.py`，并要求 `OPENAI_API_KEY`。

**发送最终回复前强制检查**：用户是否明确要求只要文字？如果没有，当前轮是否已经成功走过 Codex native `image_gen` 或用户明确选择的 CLI/API fallback？若没有，不得把文字方案当成交付。
...
- 完成构思后立即调用 Codex native `image_gen`；不要先把构思长篇输出给用户，也不要停在“最终提示词”。
...
- `references/style-gate.md`：实际生成时必读；规定 Codex native image route、canonical reference、preflight、失败条件和自动重试
...
### 3. 单张生成（Codex native 默认，CLI/API fallback）

除非用户明确要求只要文字，否则用 Codex native `image_gen` **每张单独生成**，不要拼一起。调用 skill、说“解释 X / 什么是 X / 跟这个风格一样”本身就视为生成意图，不需要用户再次说“生成”。本地参考图必须先进入上下文：对 selected primary reference 和必要 continuity anchor 使用 `view_image`，再调用 built-in `image_gen`。生成后从 `$CODEX_HOME/generated_images/...` 复制最终 PNG 到 `YC_Img_Out/`，再按任务需要复制到交接包。

只有用户明确要求 CLI/API fallback，或 Codex native `image_gen` 不可用时，才使用 `image_gen.py`。此 fallback 才需要 `OPENAI_API_KEY`，命令形态：
...
每张提示词必含：明确比例 + YC 关键词（红发 + 透明眼镜 + chibi）+ 模式画风要点 + `style-gate.md` 的全部 Sample preflight 约束。参考图必须真实进入当前生成上下文（native route 用 `view_image` / image attachment；CLI fallback 用 `--image` / input_images），不能只在 prompt 里声称“attached”。
...
Codex native `image_gen` 会先写入 `$CODEX_HOME/generated_images/...`。必须在交付前把完成的 PNG 原样复制到 `YC_Img_Out/`，并把副本作为唯一 final output path 报告；不得把工具默认输出目录当作最终交付路径。CLI/API fallback 生成时也必须直接或最终归档到同一目录。
...
如果只能产出无参考图的 prompt-only 草稿，必须标记为 `style draft / 未 reference locked`，不算正式 YC locked asset。
