...
- [ ] 已读取 `reference-routing.md` 与 `assets/examples/README.md`，并在 CLI/API input images 里实际附带与主题、比例、情绪匹配的 primary reference；不是只在文字里说有参考图
...
- [ ] 画面能看出“具体例子 → YC 操作 → 可见中间结构 → 新输入 → 具体判断”的因果
- [ ] 输入不是无差别素材堆，而是带有能被学习/处理的可见差异
- [ ] 中间不是无解释的黑箱，能看到网、筛、轨道、刻度、模板等学到/处理后的结构
- [ ] 右侧用一个新输入验证中间结构，而不是只吐出泛化的“结果”
