...
- [ ] 标注最多 2-4 个，每个 2-6 字
- [ ] 至少 40% 画面是留白
- [ ] 没有 section header / 编号列表 / 表格
- [ ] 只用了一种强调色
- [ ] YC 在执行一个核心动作（不是被信息包围）
- [ ] 去掉所有文字后，画面本身还能传达核心意思

**Simple 模式失败信号**：
- 标注超过 4 个 → 删到 2-3 个最核心的
- 出现面板/卡片分区 → 重生成，强调 NO panels, NO cards
- 出现 section header（全大写标题）→ 重生成
- 出现表格/公式列表 → 重生成，换成一个视觉隐喻
- 文字占画面面积 >20% → 文字太多，重生成

## 交付标准

**Rich 模式**：第一眼觉得「这是一个有品味的年轻创作者的笔记页」。温暖、有层次、信息完整但不像 PPT。

**Simple 模式**：第一眼觉得「画面有意思」，然后 1-2 秒看懂核心意思。温暖、留白、YC 在做一件具体的事。

两种模式都要满足：有 YC 的标志（红发+眼镜）、不像 AI 模板图。
